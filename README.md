...
CI runs lint, unit and real install tests on Ubuntu 22.04 and 24.04 (including jq 1.6 compatibility), and live firewall/DNS checks in isolated Ubuntu 24.04 and 26.04 containers. The service checks cover application profiles, repeated resolver entries, active configuration drift, SSH confirmation, and automatic rollback. Tag pushes and manual releases run the full CI suite against the requested tag before building and publishing assets. Full machine reboot behavior should additionally be exercised on disposable Ubuntu machines before a release. Requires Ubuntu, Bash 4+, and root for system changes. `prepare` installs missing configuration prerequisites from APT and, when needed, a pinned, checksum-verified yq binary; preview and status never install dependencies.
